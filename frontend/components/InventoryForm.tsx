...
export default function InventoryForm({
  ...
  const sortedHoldingsList = useMemo(() => {
  ...
  }, [holdings, sortCol, sortDir, healthData, marketData]);

  // Stable ticker key to avoid re-fetching when holdings array ref changes but tickers stay the same
  const tickerKey = useMemo(() => [...new Set(holdings.map((h) => h.ticker))].sort().join(","), [holdings]);

  const refreshMarketData = useCallback(async () => {
    const allTickers = tickerKey ? tickerKey.split(",") : [];
        ...
        <div className="mb-5">
          ...
          <form onSubmit={handleAdd} className="flex flex-wrap gap-2 items-end">
            ...
            <button
              ...
              className="shrink-0 px-3 py-2 rounded-md bg-[#21262d] hover:bg-[#30363d] border border-[#30363d] text-xs font-medium text-[#8b949e] transition-colors leading-4 flex items-center gap-1.5"
            >
              <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M16.023 9.348h4.992v-.001M2.985 19.644v-4.992m0 0h4.992m-4.993 0 3.181 3.183a8.25 8.25 0 0 0 13.803-3.7M4.031 9.865a8.25 8.25 0 0 1 13.803-3.7l3.181 3.182m0-4.991v4.99" />
              </svg>
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                type="button"
                onClick={async () => {
                  try { await clearAllHoldings(); } catch { /* ignore */ }
                  onChanged();
                }}
                title="Clear all holdings"
                className="flex items-center gap-1.5 text-[10px] font-medium text-[#f85149] hover:text-[#ff7b72] px-2 py-1 rounded transition"
              >
                <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
